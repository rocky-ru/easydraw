...
import ref from 'canvasRenderer'
import ref1 from 'svgRenderer'

const {defineCanvasRenderer,renderShapeToContext} = ref,
      {defineSVGRenderer, renderShapeToSVG} = ref1
...
const defineShape = function(name, props){
  ...
  const createShape = function(name, a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) {
    let s = new shapes[name](a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p);
    s.id = util.getGUID();
    return s;
  };

  const JSONToShape = function(arg) {
  ...
  const shapeToJSON = function(shape) {
    return {
  ...
  const bspline = function(points, order) {
    if (!order) {
      return points;
    }
  ...
  export default {
      defineShape, createShape, JSONToShape, shapeToJSON
  }
